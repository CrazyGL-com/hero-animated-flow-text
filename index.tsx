...
import CrazyGLWrapper, { loadGoogleFont, useHeroAnimationFrame, useHeroReady, type HeroComponentProps } from '@crazygl/core';
import metadata from './metadata.json';
import './style.css';

const W: Record<string, string> = { '100': '100', '200': '200', '300': '300', '400': '400', '500': '500', '600': '600', '700': '700', '800': '800', '900': '900' };

function AnimatedFlowTextHero(props: HeroComponentProps & Record<string, any>) {
	const { rootRef, reducedMotion,
		heading = 'the current carries the type', subheading = '',
		amplitude = 18, wavelength = 4, speed = 0.9, rotateMax = 6,
		textColor = '#ffffff', subColor = '#9aa1ad',
		fontSize = 88, headingFontFamily = 'Inter', headingFontWeight = '700',
	...
	return (
		<>
			...
			<crazygl-content>
				<div className="crazygl-aft-wrap" style={{ fontFamily: ff }}>
					<div className="crazygl-aft-h" style={{ fontWeight: weight, fontSize: `${Math.max(20, fontSize)}px`, color: textColor }}>
						{words.map((word, wi) => (
							<span key={wi} className="crazygl-aft-w">
								{word.map(({ i, ch }) => (
									<span key={i} ref={(el) => { refs.current[i] = el; }} className="crazygl-aft-c">{ch}</span>
								))}
							</span>
						))}
					</div>
					{String(subheading).trim() ? <div className="crazygl-aft-sub" style={{ color: subColor }}>{subheading}</div> : null}
				</div>
			</crazygl-content>
		</>
	);
...
export default function AnimatedFlowText(props: any) { return <CrazyGLWrapper hero={AnimatedFlowTextHero} metadata={metadata as any} {...props} />; }
export { metadata };
